...
LINK link_init(HardwareSerial *port)
{
  ...
  memset(link.send_queue, 0, SEND_QUEUE_SIZE * sizeof(RAW_PACKET));
...
int check_packet_complete(LINK *link)
{
  ...
  switch(preamble)
  {
    ...
    case UMPACKET_PREAMBLE:
     ...
     if(link->rbuf_writeidx >= UMPACKET_HEADER_SIZE)
      link->rbuf_expectedsize = UMPACKET_HEADER_SIZE + *((uint8_t*)&link->recvbuf[3]) + 2;  //add 2 bytes for "STX" and "ETX" for payload
...
RAW_PACKET extract_packet_from_rbuf(LINK *link)
{
     ...
     raw_packet.size = check_packet_complete(link);
     ...
     print_bytes(&link->recvbuf[0], raw_packet.size);

     //Allocate a new buffer for the raw packet for returning
     raw_packet.buf = malloc(raw_packet.size);
     memcpy(raw_packet.buf, link->recvbuf, raw_packet.size);

     link->rbuf_valid = 0;
     link->rbuf_expectedsize = 0;

     //Move write pointer to the end of the packet
     link->rbuf_writeidx -= raw_packet.size;
     memcpy(&link->recvbuf[0], &link->recvbuf[raw_packet.size], (RECV_BUFFER_SIZE - raw_packet.size));
